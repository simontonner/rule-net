...
#
# *** ADDED: Seed parameter for reproducible randomness ***
####################################################################################################
...
# *** MODIFIED: Added rng parameter for seeded randomness ***
def get_lut(indexes, labels, bits, rng):
    ...
    where_rnd = (lut == 0)
    # *** MODIFIED: Use provided rng object ***
...
# *** MODIFIED: Added rng parameter for signature consistency (not used internally) ***
def get_ripper(indexes, labels, bits, rng):
    """
...
class RipperLut:
    ...
    def __init__(self, bits, hidden_layers=[], verbose=False, mode='lut', seed: int | None = None):
        ...
        # *** ADDED: Initialize RNG with seed ***
    ...
    def train(self, X, y):
        ...
        with tqdm(self.hidden_layers, disable=not self.verbose) as t:
            ...
            for j, num_luts in enumerate(t):
                ...
                # *** MODIFIED: Use self.rng for column selection ***
                # select random columns for the current layer and remember their indices
                cols = self.rng.choice(X.shape[1] if j == 0 else self.hidden_layers[j - 1], size=num_luts*self.bits[j]).reshape((num_luts, self.bits[j]))
                ...
                # *** MODIFIED: Pass self.rng to find_rule ***
                # the starmap code has been replaced here
                tmp = np.array(
                    pool.starmap(
                        self.find_rule,
                        [[idxs[i], y_, self.bits[j], self.rng] for i in range(num_luts)], # Pass rng
                    )
                )
        ...
        # *** MODIFIED: Pass self.rng to find_rule ***
        tmp = self.find_rule(idxs, y_, self.bits[-1], self.rng) # Pass rng
    ...
    def predict(self, X):
        # No randomness involved in prediction, so no changes needed here
        assert X.dtype == bool, f"Dtype of X has to be bool, got {X.dtype}"
        N = X.shape[0]
