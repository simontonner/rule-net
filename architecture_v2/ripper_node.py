# architecture/ripper_node.py
"""
RIPPER node with exact legacy behaviour (including tie-breaking by coin flip).
Compatible with DeepBinaryClassifier.
"""
...
class RipperNode(BaseNode):
    def __init__(
        ...
        # 3. Robust coercion to np.ndarray[bool]
        if hasattr(preds, "to_numpy"):  # pandas Series
            preds = preds.to_numpy(dtype=bool)
        else:
            preds = np.asarray(preds, dtype=bool)

        # 4. Random tie-breaking for unseen entries
        if tie_break == "random":
            seen_idx = truth_table_indices(X_bits)
            unseen_mask = np.ones(2**bits, dtype=bool)
            unseen_mask[seen_idx] = False
            preds[unseen_mask] = rng.integers(0, 2, size=unseen_mask.sum(), dtype=np.int8)
