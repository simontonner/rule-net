...
def _build_node(args):
    """Executed in worker processes – creates one node."""
    node_factory, X_bits, y_pm1, bits, cols, seed, tie_break, extra_kw = args

    # 1) Create the per-node RNG
    rng = np.random.default_rng(int(seed))

    # 2) If this node_factory uses ripper_kwargs, seed it too
    extra = extra_kw.copy()
    if "ripper_kwargs" in extra:
        rk = extra["ripper_kwargs"].copy()
        rk.setdefault("random_state", int(seed))
        extra["ripper_kwargs"] = rk

    return node_factory(X_bits, y_pm1, bits, cols, rng, tie_break, **extra)
