"""Lookup-table node with majority voting + optional random tie-break."""
...
def truth_table_indices(bit_patterns: np.ndarray) -> np.ndarray:
    ...
    return (bit_patterns.astype(np.uint32) * weights).sum(axis=1).astype(np.int64)
...
# factory helper --------------------------------------------------------
def make_lut_node(X_bits, y_pm1, bits, cols, rng, tie_break):
